...
exports.createPaceEvaluation = async (req: any, res: any, next: any) => {
  ...
  try {
    ...
    res.status(error.statusCode).json({
      message: "error_creating_evaluation",
...
exports.updatePaceEvaluation = async (req: any, res: any, next: any) => {
  ...
  try {
    ...
    res.status(200).json({
      ...
      paceEvaluation,
      scores,
      message: "success_pace_evaluation_updated",
    });
  } catch (error: any) {
    await t.rollback();
    ...
    res.status(error.statusCode).json({
      message: "error_updating_evaluation",
      errorDetails: error.toString(),
      stack: error.stack,
    });
  }
...
exports.getPatientEvaluations = async (req: any, res: any, next: any) => {
  ...
  try {
    ...
    console.log(
      `${evaluations.length} évaluations trouvées pour le patient:`,
      patientId
    );
