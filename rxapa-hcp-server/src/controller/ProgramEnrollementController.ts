...
exports.createProgramEnrollement = async (req: any, res: any, next: any) => {
  ...
  try {
    ...
    res
      ...
      .json({ message: "Backend:error_creating_enrollement" });
  }
  return res;
};
...
export const createPatientWithCaregivers = async (
  ...
  try {
    ...
    if (!patientData) {
      throw new Error("Backend:error_patient_data_required");
    }

    // Validation des caregivers
    if (!caregivers || !Array.isArray(caregivers) || caregivers.length === 0) {
      throw new Error("Backend:error_at_least_one_caregiver_required");
    }

    if (caregivers.length > 2) {
      throw new Error("Backend:error_maximum_caregivers_exceeded");
    }
    ...
    if (existingUser) {
      return res.status(409).json({
        message: `Backend:error_existing_patient_email: ${patientData.email}`,
      });
    }
    ...
    for (let caregiver of caregivers) {
      ...
      if (existingCaregiver) {
        ...
        return res.status(409).json({
          message: `Backend:error_existing_caregiver_email: ${caregiver.email}`,
        });
      }
...
exports.updateProgramEnrollement = async (req: any, res: any, next: any) => {
  ...
  try {
    ...
    if (!programEnrollement) {
      return res.status(404).json({ message: "Backend:error_enrollement_not_found" });
    }
    ...
    res
      .status(error.statusCode)
      .json({ message: "Backend:error_updating_enrollement" });
  }
  return res;
};
...
exports.deleteProgramEnrollement = async (req: any, res: any, next: any) => {
  ...
  try {
    ...
    res.status(200).json({ message: "Backend:success_enrollement_deleted" });
  } catch (error: any) {
    if (!error.statusCode) {
      error.statusCode = 500;
    }
    res
      .status(error.statusCode)
      .json({ message: "Backend:error_deleting_enrollement" });
  }
  return res;
};
...
exports.getProgramEnrollement = async (req: any, res: any, next: any) => {
  ...
  try {
    ...
    res
      .status(error.statusCode)
      .json({ message: "Backend:error_loading_enrollement" });
  }
  return res;
};
...
exports.getProgramEnrollements = async (req: any, res: any, next: any) => {
  try {
    ...
    res.status(error.statusCode).json({
      message: "Backend:error_loading_enrollements",
    });
  }
  return res;
};
