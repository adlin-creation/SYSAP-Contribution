...
describe("createPatientWithCaregivers", () => {
    ...
    beforeEach(() => {
        req = {
            body: {
                ...
                caregivers: [
                    { email: 'caregiver1@example.com', program: 1, programStart: '2024-01-01', programEnd: '2024-12-31' }
                ]
            }
        };
    ...
    it("should create a patient with one caregiver successfully", async () => {
        ...
        (Patient.findOne as jest.MockedFunction<typeof Patient.findOne>).mockResolvedValue(null);
        (Caregiver.findOne as jest.MockedFunction<typeof Caregiver.findOne>).mockResolvedValue(null);

        // Mock des méthodes de création
        (Patient.create as jest.MockedFunction<typeof Patient.create>).mockResolvedValue({
            id: 1,
        ...
        (Caregiver.create as jest.MockedFunction<typeof Caregiver.create>).mockResolvedValue({
            id: 2,
            ...req.body.caregivers[0]
        });

        (ProgramEnrollement.create as jest.MockedFunction<typeof ProgramEnrollement.create>).mockResolvedValue({
            id: 3
        });

        (Patient_Caregiver.create as jest.MockedFunction<typeof Patient_Caregiver.create>).mockResolvedValue({});
    ...
    it("should create a patient with two caregivers successfully", async () => {
        ...
        req.body.caregivers = [
            { email: 'caregiver1@example.com', program: 1, programStart: '2024-01-01', programEnd: '2024-12-31' },
            { email: 'caregiver2@example.com', program: 2, programStart: '2024-01-01', programEnd: '2024-12-31' }
        ];
        ...
        (ProgramEnrollement.create as jest.MockedFunction<typeof ProgramEnrollement.create>)
        ...
        (Patient_Caregiver.create as jest.MockedFunction<typeof Patient_Caregiver.create>)
    ...
    it("should return 409 if the patient already exists", async () => {
        ...
        (Patient.findOne as jest.MockedFunction<typeof Patient.findOne>).mockResolvedValue({
            id: 1,
            email: 'patient@example.com',
            firstName: 'John',
            lastName: 'Doe'
        ...
        // Vérifications
        expect(Patient.findOne).toHaveBeenCalledWith({ where: { email: 'patient@example.com' } });
    ...
    it("should return 409 if the caregiver already exists", async () => {
        ...
        (Patient.findOne as jest.MockedFunction<typeof Patient.findOne>).mockResolvedValue(null);
        (Caregiver.findOne as jest.MockedFunction<typeof Caregiver.findOne>).mockResolvedValue({
            id: 2,
            email: 'caregiver1@example.com'
        });
        ...
        // Vérifications
        expect(Caregiver.findOne).toHaveBeenCalledWith({ where: { email: 'caregiver1@example.com' } });
        expect(res.statusCode).toBe(409);
        expect(res.json).toHaveBeenCalledWith({
    ...
    it("should handle error when creating patient", async () => {
        ...
        (Patient.create as jest.MockedFunction<typeof Patient.create>).mockRejectedValue(new Error("Patient creation failed"));
