...
describe("deleteCaregiver", () => {
  ...
  beforeEach(() => {
    ...
    const patient_Caregiver = { CaregiverId: 1, ProgramEnrollementId: 2, destroy: jest.fn() };
    const program_Enrollement = { id: 2, PatientId: 3, ProgramId: 5, destroy: jest.fn() };
    const patient = { id: 3, numberOfCaregivers: 2, numberOfPrograms: 2, save: jest.fn() };
    const programs_Enrollements_patient = [
      { dataValues: { ProgramId: 5 } },
      { dataValues: { ProgramId: 4 } }
    ];
    (Caregiver.findByPk as jest.MockedFunction<typeof Caregiver.findByPk>).mockResolvedValue(caregiver);
    (Patient_Caregiver.findOne as jest.MockedFunction<typeof Patient_Caregiver.findOne>).mockResolvedValue(patient_Caregiver);
    (ProgramEnrollement.findByPk as jest.MockedFunction<typeof ProgramEnrollement.findByPk>).mockResolvedValue(program_Enrollement);
    (Patient.findByPk as jest.MockedFunction<typeof Patient.findByPk>).mockResolvedValue(patient);
    (ProgramEnrollement.findAll as jest.MockedFunction<typeof ProgramEnrollement.findAll>).mockResolvedValue(programs_Enrollements_patient);
  ...
  it("should delete a caregiver successfully", async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({ message: "Caregiver deleted" });
  ...
  it("should return 404 if caregiver is not found", async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({ message: "Caregiver not found" });
  });
  ...
  it("should return 404 if Patient-Caregiver relation is not found", async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(404);
    expect(res.json).toHaveBeenCalledWith({ message: "Patient-Caregiver relation not found" });
  });
  ...
  it("should return 404 if Program Enrollment is not found", async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(404);
    expect(res.json).toHaveBeenCalledWith({ message: "Program Enrollement not found" });
  });
  ...
  it("should handle patient number of caregivers and programs correctly without any change in number program", async () => {
    const patient = { id: 3, numberOfCaregivers: 2, numberOfPrograms: 1, save: jest.fn() };
    Patient.findByPk.mockResolvedValue(patient);
    const programs_Enrollements_patient = [
      { dataValues: { ProgramId: 5 } },
      { dataValues: { ProgramId: 5 } }
