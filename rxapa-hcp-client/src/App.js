...
import DemandeSignup from "./components/Authentication/DemandeSignup";
  ...
  useEffect(() => {
    if (location.state?.role) {
      ...
      const filterMenuItems = (items) => {
        ...
        return items
    .map((item) => {
      // Si le rôle est kinésiologue
      if (location.state.role === "kinesiologist") {
        const allowedKeys = ["/","/patients", "/evaluations", "/programs","/calendar"];
        // Inclure uniquement les items autorisés
      ...
      if (location.state.role === "doctor") {
        const allowedKeys = ["/","/patients","/programs","/kinesiologists"];
        // Inclure uniquement les items autorisés
        if (allowedKeys.includes(item.key)) {
          return item;
        }
    ...
    return (
      <Content className="content">
        <Routes>
          ...
          <Route path="/Demande-signup" element={<DemandeSignup />} />
...
<Sider className={`sider ${role}-sidebar ${i18n.language === "ar" ? "sider-ar" : ""}`}>
  ...
  />
      ...
      >
       <Header className={`header site-layout-background ${role}-header ${i18n.language === "ar" ? "header-ar" : ""}`}>
