...
export default function PatientStats({ patient, onClose }) {
  ...
  const progressionData = hasSessions ? sessions.map((session, index) => ({
    session: index + 1,
...
const downloadCSV = () => {
  ...
  sessions.forEach((session, index) => {
    ...
    const createdAt = session.createdAt ? new Date(session.createdAt).toISOString().split('T')[0] : "";
    const updatedAt = session.updatedAt ? new Date(session.updatedAt).toISOString().split('T')[0] : "";
  ...
  if (allBlocs && allBlocs.length > 0) {
    ...
    allBlocs.forEach((bloc) => {
      ...
      const description = bloc.description ? bloc.description.replace(/"/g, '""') : "";
      const createdAt = bloc.createdAt ? new Date(bloc.createdAt).toISOString().split('T')[0] : "";
      const updatedAt = bloc.updatedAt ? new Date(bloc.updatedAt).toISOString().split('T')[0] : "";
  ...
  if (allExercises && allExercises.length > 0) {
    ...
    allExercises.forEach((exercise) => {
      ...
      const createdAt = exercise.createdAt ? new Date(exercise.createdAt).toISOString().split('T')[0] : "";
      const updatedAt = exercise.updatedAt ? new Date(exercise.updatedAt).toISOString().split('T')[0] : "";
  ...
  const needsFollowUp = hasSessions && averages && (parseFloat(averages.pain) >= 3.0 || parseFloat(averages.difficulty) >= 3.0);
  ...
  return (
    <div className="patient-data-container">
          ...
          <div className="message-container">
            {parseFloat(averages.pain) >= 3 && (
              <p className="message warning-message">
                Attention: Niveau de douleur élevé ({averages.pain}) - Consultation médicale recommandée
              </p>
            )}
            {parseFloat(averages.difficulty) >= 3 && (
              <p className="message warning-message">
                Attention: Programme trop difficile ({averages.difficulty}) - Ajustement nécessaire
              </p>
            )}
          </div>
        )}
      ...
      <h2 className="section-header">Patient Programs</h2>
      <Card>
        {hasPrograms ? (
          <div className="program-data-container">
            <div className="program-navigation">
              <Button
                onClick={() => setCurrentProgram((prev) => Math.max(prev - 1, 0))}
                disabled={currentProgram === 0}
              >
                Previous
              </Button>
              <Button
                onClick={() => setCurrentProgram((prev) => Math.min(prev + 1, patientPrograms.length - 1))}
            ...
            <div className="program-details">
              <p><strong>Program {currentProgram + 1} of {patientPrograms.length}</strong></p>
              <p><strong>Name:</strong> {currentProgramData.name}</p>
              <p><strong>Description:</strong> {currentProgramData.description}</p>
              <p><strong>Duration:</strong> {currentProgramData.duration} {currentProgramData.duration_unit}</p>
              <p><strong>Status:</strong> {currentProgramData.actif ? "Actif" : "Inactif"}</p>
            </div>
          </div>
        ) : (
          <Empty
            description={<span>{t("No programs available for this patient")}</span>}
            image={Empty.PRESENTED_IMAGE_SIMPLE}
          />
        )}
      </Card>

      <h2 className="section-header">Session Data</h2>
      <Card>
        {hasSessions ? (
          <div className="session-data-container">
            ...
            <div className="session-info">
              <div className="session-buttons">
                <Button
                  onClick={() => setCurrentSession((prev) => Math.max(prev - 1, 0))}
                  disabled={currentSession === 0}
                >
                  Previous
                </Button>
                <Button
                  onClick={() => setCurrentSession((prev) => Math.min(prev + 1, sessions.length - 1))}
              ...
              <p><strong>Walking Time:</strong> {sessionData.walkingTime}</p>
            ...
            <div className="session-chart-container">
              ...
              <div className="chart-slider-container">
                ...
                <div className="slider-with-labels">
                  <span className="slider-endpoint">First</span>
                  <Slider
                    min={0}
                    ...
                    tooltip={{
                      formatter: (value) => {
                        ...
                        return `${value + 1}: ${sessionDate.toLocaleDateString('fr-FR')}`;
                      }
                    }}
                    className="compact-slider"
                  />
                  <span className="slider-endpoint">Latest</span>
                </div>
              </div>
              ...
              <div className="session-chart">
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={sessionDataForChart}>
                    ...
                    <Bar dataKey="walkingTime" fill="#0088FE" name="Walking Time" />
      ...
      <h2 className="section-header">Average Since Inception</h2>
      <Card>
        {hasSessions ? (
          <div className="average-container">
            <div className="average-summary">
              <p><strong>Average Difficulty:</strong> {averages.difficulty}</p>
              <p><strong>Average Pain:</strong> {averages.pain}</p>
              <p><strong>Average Walking:</strong> {averages.walking}</p>
              <p><strong>Average Exercises:</strong> {averages.exercises}</p>
            </div>
            ...
            <div className="average-chart">
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={progressionData}>
                  ...
                  <Tooltip />
                  <Legend />
                  <Line type="monotone" dataKey="difficulty" stroke="#8884d8" name="Difficulty Level" />
                  <Line type="monotone" dataKey="pain" stroke="#82ca9d" name="Pain Level" />
                  <Line type="monotone" dataKey="walking" stroke="#0088FE" name="Walking Time" />
                  <Line type="monotone" dataKey="exercises" stroke="#ff7300" name="Accomplished Exercises" />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
