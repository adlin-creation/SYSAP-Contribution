...
describe("PatientViewPage Component", () => {
    ...
    it("renders the component without crashing", () => {
        ...
        expect(screen.getByText("Patients:patient_data")).toBeInTheDocument();
    });
    ...
    it("displays session data correctly", () => {
        ...
        const sessionElement = screen.getByText("Patients:session").closest("p");
        expect(within(sessionElement).getByText("2")).toBeInTheDocument();
        expect(screen.getByText("2024-03-05")).toBeInTheDocument();

        const sessionInfo = screen.getByText("Patients:session").closest(".session-info");

        expect(
            within(sessionInfo).getByText((content, element) => {
                return (
                    element.textContent.includes("Patients:difficulty_level") &&
        ...
        expect(
            within(sessionInfo).getByText((content, element) => {
                return (
                    element.textContent.includes("Patients:pain_level") &&
        ...
        expect(
            within(sessionInfo).getByText((content, element) => {
                return (
                    element.textContent.includes("Patients:accomplished_exercise") &&
    ...
    it("navigates between sessions using Previous and Next buttons", () => {
        ...
        let sessionElement = screen.getByText("Patients:session").closest("p");
        expect(within(sessionElement).getByText("2")).toBeInTheDocument();
        // Cliquez sur "Previous"
        fireEvent.click(screen.getByText("Patients:button_previous"));

        sessionElement = screen.getByText("Patients:session").closest("p");
        expect(within(sessionElement).getByText("1")).toBeInTheDocument();
        expect(screen.getByText("2024-03-01")).toBeInTheDocument();

        const sessionInfo = screen.getByText("Patients:session").closest(".session-info");
        const difficultyLevelElement = within(sessionInfo)
            .getByText("Patients:difficulty_level")
            .closest("p");
        ...
        const accomplishedExerciseElement = within(sessionInfo)
            .getByText("Patients:accomplished_exercise")
            .closest("p");
        expect(accomplishedExerciseElement).toHaveTextContent("4");

        // Cliquez sur "Next"
        fireEvent.click(screen.getByText("Patients:button_next"));
        sessionElement = screen.getByText("Patients:session").closest("p");
        expect(within(sessionElement).getByText("2")).toBeInTheDocument();
    });
