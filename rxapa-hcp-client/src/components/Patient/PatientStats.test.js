...
describe("PatientStats Component", () => {
    beforeEach(() => {
        ...
        useQuery.mockImplementation((queryKey) => {
            ...
            if (queryKey[0] === "SessionRecord") {
                ...
                return { data: mockProgramEnrollments, isLoading: false, isError: false };
    ...
    it("displays patient information correctly", () => {
        ...
        expect(screen.getByText(/Patients:programs/)).toBeInTheDocument();
    });
    ...
    it("displays session data correctly", () => {
        ...
        expect(screen.getByText(/Accomplished Exercice:/i)).toBeInTheDocument();
    });
    ...
    it("navigates between sessions using Previous and Next buttons", () => {
        ...
        const sessionSection = screen.getByText("Session Data").closest('.section-header').nextElementSibling;
        const sessionButtons = within(sessionSection).getAllByRole('button');
        
        // Get prev/next buttons
        const prevButton = sessionButtons.find(btn => /previous/i.test(btn.textContent));
        const nextButton = sessionButtons.find(btn => /next/i.test(btn.textContent));
        ...
        const updatedSessionSection = screen.getByText("Session Data").closest('.section-header').nextElementSibling;
        const updatedButtons = within(updatedSessionSection).getAllByRole('button');
        
        const updatedPrevButton = updatedButtons.find(btn => /previous/i.test(btn.textContent));
        const updatedNextButton = updatedButtons.find(btn => /next/i.test(btn.textContent));
    ...
    it("displays averages section when sessions exist", () => {
        ...
        expect(screen.getByText("Average Since Inception")).toBeInTheDocument();
        
        expect(screen.getByText(/Average Difficulty:/i)).toBeInTheDocument();
        expect(screen.getByText(/Average Pain:/i)).toBeInTheDocument();
        expect(screen.getByText(/Average Walking:/i)).toBeInTheDocument();
        expect(screen.getByText(/Average Exercises:/i)).toBeInTheDocument();
    });
    ...
    it("shows empty state when no sessions exist", () => {
        ...
        const emptyMessages = screen.getAllByText("Aucune session disponible pour ce patient");
    ...
    it("displays slider for session navigation", () => {
      ...
      const firstLabel = sessionSection.querySelector('.slider-endpoint, .slider-labels');
      expect(firstLabel).not.toBeNull();
      
      // Check if at least one of the endpoints contains "First" and one contains "Latest"
      const endpointTexts = Array.from(sessionSection.querySelectorAll('.slider-endpoint, .slider-labels span'))
          .map(el => el.textContent);
      
      expect(endpointTexts.some(text => text.includes('First'))).toBe(true);
      expect(endpointTexts.some(text => text.includes('Latest'))).toBe(true);
  })
});